...
# Ajustement du modèle COM avec lien cloglog ----------------------------------------

modele <- glm(maturite ~ ltm : sexe, family = binomial(link=cloglog), data=df_l50)
...
b1 <- coef(modele)[["ltm:sexeF"]]
b2 <- coef(modele)[["ltm:sexeM"]]

# Estimation de l50 à partir des coefficients

l50_F <- (-b0 - 0.3665129) / b1
l50_M <- (-b0 - 0.3665129) / b2
...
DATAogive <- newDF %>%
  mutate(
    maturite = 1-exp(-exp(newDFpred$fit)),
    lim_inf = 1-exp(-exp(newDFpred$fit - (1.96 * newDFpred$se.fit))),
    lim_sup = 1-exp(-exp(newDFpred$fit + (1.96 * newDFpred$se.fit)))
  )
...
minitable<- rbind(
  ...
  c("sexe", round(b2, digits = 3))) %>% as.data.frame()
