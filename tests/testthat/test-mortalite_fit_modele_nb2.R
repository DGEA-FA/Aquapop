test_that("mortalite_fit_modele_nb2 retourne un tableau structuré dans le cas nominal", {
  ...
  # Méthode attendue
  expect_equal(res$methode, "nb2")
  
  # Si le modèle ne converge pas ou que les paramètres ne sont pas exploitables
  # avec ces données simulées, on saute les vérifications numériques spécifiques
  if (!isTRUE(res$convergence)) {
    skip("Le modèle NB2 n'a pas produit un résultat exploitable avec ces données simulées")
  }
...
test_that("mortalite_fit_modele_nb2 retourne une ligne d'échec si la colonne age est absente", {
...
test_that("mortalite_fit_modele_nb2 retourne une ligne d'échec si la colonne number est absente", {
...
test_that("mortalite_fit_modele_nb2 retourne une ligne d'échec si moins de deux âges distincts sont disponibles", {
  df <- tibble::tibble(
    age = c(5, 5, 5),
    number = c(10, 12, 8)
  )
