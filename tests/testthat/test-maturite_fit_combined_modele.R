test_that("Retourne une liste de 12 modèles glm pour ltm", {
  df <- tibble::tibble(
...
test_that("Retourne une liste de 12 modèles glm pour age", {
  df <- tibble::tibble(
    maturite = rep(0:1, each = 10),
    sexe = factor(rep(c("M", "F"), each = 10), levels = c("M", "F")),
    age = rep(1:10, 2)
...
test_that("Erreur si colonnes obligatoires manquantes", {
  ...
  expect_error(maturite_fit_combined_modele(df1, "ltm"), "maturite")
  expect_error(maturite_fit_combined_modele(df2, "ltm"), "sexe")
  expect_error(maturite_fit_combined_modele(df3, "ltm"), "ltm")
})
