test_that("maturite_compare_modele retourne une liste structurée avec modèles séparés par défaut", {
  ...
  expect_type(res$table_sep$df$aicc, "double")
  expect_type(res$table_sep$df$pearson_x2_pval, "double")
  expect_type(res$table_sep$df$goodness_of_link_pval, "double")
  
  expect_type(res$table_comb$df$aicc, "double")
  expect_type(res$table_comb$df$pearson_x2_pval, "double")
  expect_type(res$table_comb$df$goodness_of_link_pval, "double")
...
test_that("maturite_compare_modele peut retourner le tableau principal combiné si prefer_combined = TRUE", {
  ...
  if (!is.null(res$best_model$best_model_combined)) {
    ...
    expect_equal(
      res$best_model$best_model_combined$variable,
      "age"
    )
  }
...
test_that("maturite_compare_modele retourne success = FALSE si aucun modèle n'est possible faute de variation de maturité", {
  ...
  expect_type(res$table$df$convergence, "logical")
  expect_type(res$table$df$aicc, "double")
  expect_type(res$table$df$pearson_x2_pval, "double")
  expect_type(res$table$df$goodness_of_link_pval, "double")
  expect_type(res$table$df$recommande, "logical")
