#' Ajuste le modèle TLO_cloglog (sexes combinés)
#'
...
#' @export
modele_TLO_cloglog <- function(data, nboot = 10000) {
  ...
  modele <- glm(maturite ~ ltm, family = binomial(link = "cloglog"), data = df)
  ...
  minitable_flextable <- minitable %>%
    ...
    flextable::set_caption("Résumé du modèle TLO_cloglog (sexes combinés)")
  ...
  plot <- ggplot(DATAogive, aes(x = ltm, y = maturite)) +
    ...
    theme_classic() +
    labs(title = "Ogive de maturité (TLO_cloglog, sexes combinés)",
