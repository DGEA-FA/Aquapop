biomasse_table <- function(capture, specimen, espece) {
  ...
  alldata <-
    alldata %>% dplyr::filter(type_maill %in% c("G", NA))  %>% droplevels() #on veut juste les filets expérimentaux
...
# tous --------------------------------------------------------------------
  ...
  mp <-
    glm(BPUE ~ 1, family = poisson, data = temp)#modele glm initial pour le calcul
  ...
  resultBPUE <-
    cbind(
      newdata,
      as.numeric(BPUEfinal),
      as.numeric(lowerlimBPUE),
      as.numeric(upperlimBPUE),
      method
    )
  ...
  # MFIND -------------------------------------------------------------------
  temp <-
    alldata %>% dplyr::group_by(no_station, sexe) %>% droplevels() %>%
  ...
  # FMmature ----------------------------------------------------------------
  temp <-
    alldata %>% filter(maturite == "O" &
                         sexe %in% c("M", "F")) %>% droplevels() #slmt poissons matures de sexe connu
  ...
  temp <-
    alldata %>% filter(maturite == "N") %>% droplevels() #slmt poissons Imm. ou reprod. inactifss
  ...
  temp <-
    temp %>% dplyr::select(c(maturite, Biomasse, Perc, BPUE, IC95))
  ...
  temp <- temp %>% dplyr::group_by(no_station) %>%
    summarise(massesum = sum(masse, na.rm = TRUE))
  temp <-
    temp %>% summarise(Biomasse = round(sum(massesum, na.rm = TRUE) / 1000, digits =
                                          1),
                       BPUE = round((sum(massesum, na.rm = TRUE) /
                                       nstation) / 1000, digits = 1))
  temp <- temp %>% mutate(Perc = Biomasse * 100 / biomassefix,
                          IC95 = NA)
  temp <- temp %>% mutate(Perc = round(Perc, digits = 0))
  ...
  CLEAN <-
    CLEAN %>%  dplyr::select(c(Groupe, everything())) #Moving the last column to the start
  CLEAN$Perc[CLEAN$Groupe == "Tous"] <- 100
  CLEAN$Perc <-
    format(round(as.numeric(CLEAN$Perc), digits = 0), nsmall = 0)
  CLEAN
}