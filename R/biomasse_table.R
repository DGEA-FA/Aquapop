...
Tous <- as.data.frame(c(
          ...
          Perc = NA,
          BPUE = BPUEfinal,
          "IC 95%" = paste0("(",lowerlimBPUE,"-",upperlimBPUE,")")))
...
temp <- temp %>% dplyr::select(c(sexe,Biomasse,Perc,BPUE,"IC 95%"))
...
#Immature
temp <- alldata %>% filter(maturite=="N") %>% droplevels() #slmt poissons immatures
...
temp <- temp %>% mutate(Perc = Biomasse*100/biomassefix,
                        "IC 95%" =NA)
temp <- temp %>% mutate(Perc = round(Perc, digits = 0))
temp <- temp %>% dplyr::select(c(maturite, Biomasse, Perc, BPUE, "IC 95%"))

temp <- temp %>% dplyr::select(-c(maturite))

temp <- t(temp) %>% as.data.frame()
colnames(temp) <- "Immature"

Immature <- mutate_all(temp, function(x) as.numeric(as.character(x)))  #CLEAN


CLEAN <- cbind(Tous, MFIND, FMmature, Immature)

#IDfix <- unique(capture$ID) %>% as.character()
#CLEAN <- CLEAN %>% mutate(ID= paste(IDfix))

CLEAN <-  CLEAN %>% dplyr::select(c("Tous", "Femelle", "Mâle", "♀ mature","♂ mature", "Immature", "sexe inconnu"))
CLEAN <- t(CLEAN) %>% as.data.frame()
CLEAN <- CLEAN %>% mutate(Groupe =c("Tous", "Femelle", "Mâle", "♀ mature","♂ mature", "Immature", "sexe inconnu"))
