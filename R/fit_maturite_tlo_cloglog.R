#' Ajuste le modèle TLO_cloglog (sexes combinés) pour L50 ou A50
#'
...
#' @export
fit_maturite_tlo_cloglog <- function(data, variable = c("ltm", "age"), nboot = 10000) {
...
# Ajustement du modèle ----------------------------------------------------
  if (variable == "ltm") {
    modele <- sans_warning_proba(
      glm(maturite ~ ltm, family = binomial(link = "cloglog"), data = donnees_modeles))
  } else {
    modele <- sans_warning_proba(
      glm(maturite ~ age, family = binomial(link = "cloglog"), data = donnees_modeles))
  }
  ...
  donnees_ogive <- donnees_prediction %>%
    mutate(
      maturite = 1 - exp(-exp(prediction_model$fit)),
      lim_inf = 1 - exp(-exp(prediction_model$fit - 1.96 * prediction_model$se.fit)),
      lim_sup = 1 - exp(-exp(prediction_model$fit + 1.96 * prediction_model$se.fit))
    )
