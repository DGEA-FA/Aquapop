courbe_croissance_ggVONBERT <- function(initcroissance, tablemodele) {
 ...
 dfbase <- Summarize(ltm~age,data=init)  #truc de FSA pour avoir la ltm moy par age
 ...
 #CHANGÉ PAR ROX, SINON NE TROUVE PAS LA FONCTION vb DANS SHINY
 ...
 preds <- data.frame(age=ageGRAPHbreak,
   ...
   geom_line(data=dplyr::filter(preds,age>=agemin,age<=agemax),aes(x=age,y=fit),linewidth=1) +
   scale_y_continuous(name="Longueur totale maximale (mm)",expand=c(0,0)) +
   scale_x_continuous(name="Âge (année)",
