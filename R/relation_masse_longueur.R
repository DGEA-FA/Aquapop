relation_masse_longueur <- function(data, espece) {
  ...
  dfAllometrie <- data  %>% filter(sp==espece) %>% droplevels() #seulement l'espece PEN
  
  dfAllometrie  <- subset(dfAllometrie, !is.na(ltm))# this data frame needed to be “cleaned” by removing all records where mesures ltm were missing
  dfAllometrie <- subset(dfAllometrie, !is.na(masse))# this data frame needed to be “cleaned” by removing all records where mesures masse were missing 
  ...
  fit1 <- lm(logW~logL,  data = dfAllometrie ) #simple linear regression fitted with lm
  a <- coef(fit1)[1] %>% as.numeric() %>% round(digits = 3) #extration des parametres du modele #UPDATE CEST LOG 10 de A
  b <- coef(fit1)[2] %>% as.numeric() %>% round(digits = 3) #extration des parametres du modele 
  
  #visualising the fit
  tmp <- range(dfAllometrie$logL) #etendue des valeurs de logL (get min et max)
  xs <- seq(tmp[1],tmp[2],length.out=99)  #get sequence de valeurs de min a max by 1 (pour l'axe des X)
  ys <- predict(fit1,data.frame(logL=xs)) #predire val de Y pour valeurs de X avec le modele fit1 calcule
  ...
  btys <-  cf*10^predict(fit1,data.frame(logL=xs), interval="prediction") # same as Ogle p.138 IFAR
  btxs <- 10^xs
...
#faire le graphique. Inspirée de Ogle p.139 IFAR, mais que jai traduit en ggplot2  
 ...
 # Labeltext <- map(paste('<b>log10a = </b>', a, '<b>b = </b>', b,'<br>'), HTML)
  ...
  ggRelationML <- 
    ggplot() + 
    geom_point(data = dfAllometrie, aes(x=LTmax, y=Masse, text=map(paste('<b># spécimen:</b>', no_specimen, '<br>'), HTML))) + 
    ...
    annotate("text", label=paste0("log10a = ", a,
                                  ", b = ", b), x = min(dfAllometrie$LTmax)+200, y =  max(dfAllometrie$Masse),color="black", vjust = "inward", hjust = "inward")
  ggRelationML
  }
