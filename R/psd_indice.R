psd_indice <- function(data, sp) {
 ...
 # ID <- unique(bunch$ID)
  ...
  if (temp == 4) {
    PSDresult  <- FSA::psdCI(c(0,1,1,1),ptbl = psdtable , n=sum(gfreq ), method = "binomial", label="PSD Q") %>% as.data.frame()
  }
  if (temp == 5) {
    PSDresult <- FSA::psdCI(c(0,1,1,1,1),ptbl = psdtable , n=sum(gfreq ), method = "binomial", label="PSD Q") %>% as.data.frame()
  }
  if (temp == 3) {
    PSDresult <- FSA::psdCI(c(0,1,1),ptbl = psdtable , n=sum(gfreq ), method = "binomial", label="PSD Q") %>% as.data.frame()
  }
  if (temp == 2) {
    PSDresult <- FSA::psdCI(c(0,1),ptbl = psdtable, n=sum(gfreq), method = "binomial", label="PSD Q") %>% as.data.frame()
  }
...
#  PSDresult <- PSDresult %>% mutate(ID=ID)
  PSDresult <-PSDresult %>% dplyr::select(#ID,
    1, 4)
  }