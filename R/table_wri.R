table_wri <- function(data, espece) {
  ...
  pred2 <-
    predict(aov2 , nd2 , interval = "confidence") %>% as.data.frame()
  ...
  y3 <-
    init %>%  summarise(n = n()) %>% droplevels()#nb de poissons dans ch classe de taille
  
  nd3 <- data.frame(Groupe = "Tous")
  pred3 <-
    predict(aov3 , nd3 , interval = "confidence") %>% as.data.frame()
  ...
  CLEAN <- CLEAN %>% rename(Wr = fit,
                            "IC 95%" = IC95)
  CLEAN <-
    CLEAN %>% mutate(Groupe = plyr::mapvalues(
      Groupe,
      from = c("F", "M"),
      to = c("Femelle", "Mâle")
    ))
