minitable_param_model_int_l50 <- function(model, df) {
  ...
  modelM <-
    glm((as.numeric(maturite) - 1) ~ ltm,
        data = dfM,
        family = binomial(link = LINKK))
  l50arrayM <- as.array(dose.p(modelM, cf = 1:2, p = 0.5))
  L50M <-
    format(round(as.numeric(l50arrayM), 2), nsmall = 2) %>% as.numeric() #arrondir a 2 decimales
  sel50M <-
    format(round(as.numeric(attr(l50arrayM, "SE")), 2), nsmall = 2) %>% as.numeric()
  ...
  L50F <-
    format(round(as.numeric(l50arrayF), 2), nsmall = 2) %>% as.numeric() #arrondir a 2 decimales
  sel50F <-
    format(round(as.numeric(attr(l50arrayF, "SE")), 2), nsmall = 2) %>% as.numeric()
