#' Calcule ou visualise l'indice de condition (Wr) des spécimens d'une espèce
#'
#' Produit un tableau synthèse (Wr moyen + IC95) ou un graphique de dispersion par sexe (`plot_tous`)
#' ou par classe de taille (`plot_byclass`) pour une espèce donnée.
#'
#' @param data Données brutes de spécimens (`data.frame` avec colonnes `sp`, `ltm`, `masse`, etc.)
#' @param format Format de sortie : `"data.frame"`, `"flextable"`, `"plot_tous"` ou `"plot_byclass"`
#'
#' @return Un `data.frame`, un `flextable`, ou un `ggplot` selon le format demandé.
#' @export
#'
#' @examples
#' indice_condition(data = specimen, format = "data.frame")
#' indice_condition(data = specimen, format = "plot_tous")
indice_condition <- function(data,
                             format = c("data.frame", "flextable", "plot_tous", "plot_byclass")) {
  ...
  if (format == "plot_tous") {
    ...
    return(
      ggplot2::ggplot(data, ggplot2::aes(x = ltm, y = Wri, color = sexe)) +
  ...
  if (format == "plot_byclass") {
    ...
    # Utiliser seulement les niveaux présents dans le modèle
    gcat_obs <- levels(droplevels(data$gcat))  # niveaux réellement observés
    ...
    return(
      ggplot2::ggplot(pred, ggplot2::aes(x = Classe, y = fit)) +
        ggplot2::geom_point() +
  ...
  # --- Table synthèse ---
  classe <- FSA::lencat(data$ltm, breaks = info$breaks)
  data <- data |>
    dplyr::mutate(
      Classe = dplyr::recode(as.character(classe), !!!setNames(psd_classnames, as.character(info$breaks))),
      Intervalle = dplyr::recode(as.character(classe), !!!setNames(info$break_labels, as.character(info$breaks)))
    )
  ...
  final <- dplyr::bind_rows(tab_all, tab_sexe, tab_class)
  
  if (format == "data.frame") return(final)
