minitable_param_model_add_a50 <- function(model) {
  ...
  #library(car)
  LI_F <-
    deltaMethod(coef(model), "-(Intercept)/age", vcov. = vcov(model))$`2.5 %` %>% round(digits = 0)
  LS_F <-
    deltaMethod(coef(model), "-(Intercept)/age", vcov. = vcov(model))$`97.5 %`  %>% round(digits = 0)
  LI_M <-
    deltaMethod(coef(model), "-(Intercept+sexeM)/(age)", vcov. = vcov(model))$`2.5 %`  %>% round(digits = 0)
  LS_M <-
    deltaMethod(coef(model), "-(Intercept+sexeM)/(age)", vcov. = vcov(model))$`97.5 %` %>% round(digits = 0)
