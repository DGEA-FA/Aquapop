minitable_param_model_int_a50 <- function(model, df) {
  ...
  modelM <-
    glm((as.numeric(maturite) - 1) ~ age,
        data = dfM,
        family = binomial(link = LINKK))
  a50arrayM <- as.array(dose.p(modelM, cf = 1:2, p = 0.5))
  A50M <-
    format(round(as.numeric(a50arrayM), 2), nsmall = 2) %>% as.numeric() #arrondir a 2 decimales
  sea50M <-
    format(round(as.numeric(attr(a50arrayM, "SE")), 2), nsmall = 2) %>% as.numeric()
  ...
  A50F <-
    format(round(as.numeric(a50arrayF), 2), nsmall = 2) %>% as.numeric() #arrondir a 2 decimales
  sea50F <-
    format(round(as.numeric(attr(a50arrayF, "SE")), 2), nsmall = 2) %>% as.numeric()
