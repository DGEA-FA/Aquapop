...
temp1 <- datalac %>% summarise("Type de pêche" = unique(typ_pech),
...
temp2 <- capture %>% summarise("Date de début de l’inventaire (aaaa-mm-jj)" = paste(min(date_pose)),
                               "Date de fin de l’inventaire (aaaa-mm-jj)" = paste(max(date_leve))   )

temp3 <- capture %>% filter(st_hasard =="O") %>% summarise("N stations aléatoires" = n( ))

temp4 <- capture %>% filter(st_hasard =="N") %>% summarise("N stations dirigées" = n( ))
