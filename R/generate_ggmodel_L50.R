generate_ggmodel_L50 <- function(df, model, minitable, model_type) {
  ...
  if (model_type == "L") {
    ...
    plot <- ggplot(data = DATAogive, aes(x = ltm, y = maturite)) +
      ...
      annotate("segment", x = L50, xend = L50, y = 0, yend = 0.5, colour = "black", lty = 2) +
      annotate("segment", x = min(DATAogive$ltm), xend = L50, y = 0.5, yend = 0.5, colour = "black", lty = 2)
    ...
    plot <- ggplot(data = DATAogive, aes(x = ltm, y = maturite, color = sexe)) +
      ...
      annotate("segment", x = min(DATAogive$ltm), xend = L50_M, y = 0.5, yend = 0.5, colour = "#bdbdbd", lty = 2) +
      annotate("segment", x = L50_F, xend = L50_F, y = 0, yend = 0.5, color = "#636363", lty = 2) +
      annotate("segment", x = min(DATAogive$ltm), xend = L50_F, y = 0.5, yend = 0.5, colour = "#636363", lty = 2)
  }
  
  plot <- plot + theme_classic() +
    labs(x = "Longueur totale maximale (mm)", y = "Proportion reproducteur actif") +
    geom_point(data = df, mapping = aes(x = ltm, y = as.numeric(maturite) - 1, color = sexe)) +
