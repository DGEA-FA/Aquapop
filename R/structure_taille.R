structure_taille <- function(data, espece, regroupement) {
  ...
  #df <- df %>% filter(caract_eng=="Exp") %>% droplevels() #sélectionner slmt les specimens capture avec filet experimental
  df <- subset(df, !is.na(ltm))# this data frame needed to be “cleaned” by removing all records where mesures were missing
  max <- max(df$ltm ) # définir la plus grande valeur de ltm, il faudra verifier si ça fitte avec le tableau abondance
  ...
  axeY <- paste0("N des ", nomsp, " échantillonnés")
    ...
    geom_histogram(binwidth = binwidth, closed= "right",#fill="black",
    ...
    scale_x_continuous(expand = c(0.1, 0.1),
                       breaks = seq(from = 0, to = max+(binwidth*2), by = binwidth)) +
