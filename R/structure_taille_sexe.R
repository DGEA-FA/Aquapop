structure_taille_sexe <- function(dfspecimen, espece, binwidth, nomsp) {
  ...
  df <- df %>% select(ltm, sexe)
   ...
   # Création de la ligne avec sexe "F"
   new_row_F <- data.frame(
     ltm = max_ltm,
     sexe = "F"
   )
   
   # Création de la ligne avec sexe "M"
   new_row_M <- data.frame(
     ltm = max_ltm,
     sexe = "M"
   )
   
   # Création de la ligne avec sexe "IND"
  ...
  ggplot(df, aes(x = ltm, fill = sexe)) +
    ...
    geom_histogram(data = dfnew, binwidth = binwidth,aes(x = ltm, fill = sexe), alpha = 0,  na.rm = TRUE ) +
