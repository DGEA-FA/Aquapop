fig_wri_byclass <- function(data, espece) {
  ...
  init <-
    init  %>% dplyr::select(no_specimen, sp, ltm, masse, age, sexe, maturite) #selectionner slmt les col necessaires
  ...
  if (unique(init$sp %>% droplevels()) %>% as.character() == "SANA") {
    init  <-
      mutate(init , range = plyr::mapvalues(
        gcat,
        from = breakClass,
        to = c(
          "<300",
  ...
  pred <-
    predict(aov1 , nd , interval = "confidence") %>% as.data.frame()
  pred <- cbind(pred, nd)
  pred <- merge(pred,
                y, by = "gcat")
  ...
  pred %>% arrange(gcat) %>% ggplot(aes(x = Classe, y = fit)) +
    ...
    scale_y_continuous(breaks = scales::breaks_extended(n = 5, only.loose =
                                                          TRUE)) +
    scale_x_discrete(
      breaks = Classename,
      drop = FALSE,
      limits = c(
        ...
        "Mémorable",
        "Trophée"
      ) # Use limits to adjust in what order
    ) +
    annotate(
      "segment",
      x = -Inf,
